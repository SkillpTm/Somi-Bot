    ...
    async def custom_add(
        ...
        *,
        ...
        if not re.match(r"^[a-z0-9]+$", name):
            await interaction.followup.send(embed=EmbedFunctions().get_error_message("You can only have letters and numbers in your custom-name!"))
            return
        ...
        await interaction.followup.send(embed=EmbedFunctions().get_success_message(f"Your custom-command with the name `{name}` has been created."))
