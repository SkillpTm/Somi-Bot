...
class KeywordDelete(nextcord_C.Cog):
    ...
    async def keyword_delete(
        ...
        *,
        ...
        if not keyword and not delete_all:
            await interaction.followup.send(embed=EmbedFunctions().get_error_message("Please either provide a keyword or choose to delete all your keywords."))
            return
        ...
        await interaction.followup.send(embed=EmbedFunctions().get_success_message(f"`{keyword}` has been deleted from your keywords."))
    ...
    async def delete_all(self, interaction: nextcord.Interaction[SomiBot]) -> None:
        ...
        await interaction.followup.send(embed=EmbedFunctions().get_info_message("Do you really want to delete **ALL** your keywords __**(they can't be recovered)**__?"), view=view)
        await view.wait()

        if not view.value:
            await interaction.followup.send(embed=EmbedFunctions().get_error_message("Your keywords have **not** been deleted!"))
            return
        ...
        await interaction.followup.send(embed=EmbedFunctions().get_success_message("**ALL** your keywords have been deleted!"))
