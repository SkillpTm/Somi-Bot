    ...
    async def on_message(self,
        ...
        for user_id, user_keywords in all_users_keywords.items():
            ...
            for keyword in user_keywords:
                if f" {keyword} " in re.sub('<[^ ]+?>', "", message.content.lower()) or re.sub('<[^ ]+?>', "", message.content.lower()).startswith(f"{keyword} ") or re.sub('<[^ ]+?>', "", message.content.lower()).endswith(f" {keyword}") or re.sub('<[^ ]+?>', "", message.content.lower()) == f"{keyword}": # regex -> message without emotes
