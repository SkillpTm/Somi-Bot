...
class LastFmArtist(nextcord_C.Cog):
    ...
    async def lastfm_artist(
        ...
        *,
        ...
        artist_response = requests.get(f"https://www.last.fm/user/{lastfm_username}/library/music/{artist_for_url}?date_preset={timeframe}", cookies=self.client.keychain.LAST_FM_COOKIES, headers=self.client.keychain.LAST_FM_HEADERS, timeout=10)
