    ...
    async def ban(
        ...
        *,
        ...
        if interaction.user.top_role.position < member.top_role.position and interaction.user != interaction.guild.owner: # type: ignore
            await interaction.followup.send(embed=EmbedFunctions().get_error_message("You can only ban a member, if your current top-role is above their current top-role!"))
            return

        await interaction.guild.ban(user=member, reason=reason, delete_message_seconds=delete_message_hours * 60 * 60)

        await interaction.followup.send(embed=EmbedFunctions().get_success_message(f"Succesfully banned {member.mention}."))
    ...
    async def unban(
        ...
        *,
        ...
        if not user_id.isdigit():
            await interaction.followup.send(embed=EmbedFunctions().get_error_message(f"`{user_id}` isn't a valid discord user id."))
            return
        ...
        try:
            ...
            await interaction.followup.send(embed=EmbedFunctions().get_success_message(f"{user.mention} has been unbanned."))
        except nextcord.Forbidden:
            await interaction.followup.send(embed=EmbedFunctions().get_error_message(f"{user.mention} wasn't unbanned."))
            return
