...
class Send(nextcord_C.Cog):
    ...
    async def send(
        ...
        *,
        ...
        await interaction.followup.send(embed=EmbedFunctions().get_success_message(f"Message sent in: {channel.mention} - [Link]({message_object.jump_url})"))
    ...
    async def edit(
        ...
        *,
        ...
        if not (message_object := await self.client.get_message_from_link(link)):
            await interaction.followup.send(embed=EmbedFunctions().get_error_message(f"`{link}` isn't a valid message link."))
            return

        if message_object.author.id != self.client.user.id or message_object.guild.id != interaction.guild.id:
            await interaction.followup.send(embed=EmbedFunctions().get_error_message(f"`{link}` isn't the link of a message sent by the bot in this server."))
            return

        await message_object.edit(content=message)
        await interaction.followup.send(embed=EmbedFunctions().get_success_message(f"Message edited in: {message_object.channel.mention} - [Link]({message_object.jump_url})")) # type: ignore
