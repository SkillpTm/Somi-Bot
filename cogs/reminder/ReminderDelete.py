...
class ReminderDelete(nextcord_C.Cog):
    ...
    async def reminder_delete(
        ...
        *,
        ...
        if not reminder_id and not delete_all:
            await interaction.followup.send(embed=EmbedFunctions().get_error_message("Please either provide an ID or choose to delete all your reminders."))
            return

        if not await db.Reminder._.get_all(where={db.Reminder.USER: interaction.user.id}):
            await interaction.followup.send(embed=EmbedFunctions().get_error_message("You don't have any reminders to be deleted!"))
            return
        ...
        if not reminder_id.isdigit():
            await interaction.followup.send(embed=EmbedFunctions().get_error_message(f"`{reminder_id}` isn't a valid reminder id."))
            return
        ...
        await interaction.followup.send(embed=EmbedFunctions().get_success_message(f"Your reminder `{reminder_id}` has been deleted."))
    ...
    async def delete_all(self, interaction: nextcord.Interaction[SomiBot]) -> None:
        ...
        if not view.value:
            await interaction.followup.send(embed=EmbedFunctions().get_error_message("Your reminders have **not** been deleted!"))
            return
