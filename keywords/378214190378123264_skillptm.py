...
class skillptm_378214190378123264(commands.Cog):
  ...
  async def on_message(self, ctx):
    ...
    if ctx.guild:
      ...
      if ctx.channel.id == 829871264982106182:
        return
      if "skillptm" in re.sub(":.*?:", "", str(ctx.content.lower())):
        link = "https://discord.com/channels/" + str(ctx.guild.id) + "/" + str(ctx.channel.id) + "/" + str(ctx.id)
        embed = Embed(title="Keyword notification: `skillptm`",
        ...
        if len(ctx.content) > 910:
          fields = [("Reason:", "Your keyword: `skillptm` has been said in <#" + str(ctx.channel.id) + "> by <@" + str(ctx.author.id) + ">:", False),
...
def setup(client):
  client.add_cog(skillptm_378214190378123264(client))