...
class ryu_412313156975853588(commands.Cog):
  ...
  async def on_message(self, ctx):
    ...
    if ctx.guild:
      ...
      if ctx.channel.id == 829871264982106182:
        return
      if "ryu" in re.sub(":.*?:", "", str(ctx.content.lower())):
        link = "https://discord.com/channels/" + str(ctx.guild.id) + "/" + str(ctx.channel.id) + "/" + str(ctx.id)
        embed = Embed(title="Keyword notification: `ryu`",
        ...
        if len(ctx.content) > 910:
          fields = [("Reason:", "Your keyword: `ryu` has been said in <#" + str(ctx.channel.id) + "> by <@" + str(ctx.author.id) + ">:", False),
...
def setup(client):
  client.add_cog(ryu_412313156975853588(client))