...
class somi_412313156975853588(commands.Cog):
  ...
  async def on_message(self, ctx):
    user = await self.client.fetch_user("412313156975853588")
    if str(ctx.author.id) == "412313156975853588":
      return
    if ctx.author.bot:
      return
    if ctx.guild:
      ...
      if ctx.channel.id == 829871264982106182:
        return
      if "somi" in re.sub(":.*?:", "", str(ctx.content.lower())):
        link = "https://discord.com/channels/" + str(ctx.guild.id) + "/" + str(ctx.channel.id) + "/" + str(ctx.id)
        embed = Embed(title="Keyword notification: `somi`",
        ...
        if len(ctx.content) > 910:
          fields = [("Reason:", "Your keyword: `somi` has been said in <#" + str(ctx.channel.id) + "> by <@" + str(ctx.author.id) + ">:", False),
...
def setup(client):
  client.add_cog(somi_412313156975853588(client))